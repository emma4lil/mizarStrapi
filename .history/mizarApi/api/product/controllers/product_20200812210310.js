...
module.exports = {
  findByTag: async (ctx) => {
    let data = await strapi.services.product.findProductsByTag(ctx.params);
    data = data.map((entity) =>
      sanitizeEntity(entity, { model: strapi.models.product })
    );
    return deepFilter(data, { key: "tags.tag", val: ctx.params.tag });
  },
  // find by Filter: product.specification
  findByFilter: async (ctx) => {
    const { attr } = ctx.params;
    let data = await strapi.services.product.findByProductsFilter({});
    data = data.map((entity) =>
      sanitizeEntity(entity, { model: strapi.models.product })
    );
    return data;
